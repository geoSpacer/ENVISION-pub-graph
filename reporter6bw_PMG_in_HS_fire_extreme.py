...
##    ownersToGraph = ['Federal','State','Private Non-Industrial','Private Industrial','Tribal','Homeowner']
    ...
    if ownership == 'All':
        pdfFile = PdfPages(outDir + chartTitle + '_HS_Fire_extreme10.pdf')
    elif ownership in ownersToGraph:
        ownersToGraph = [ownership]
        pdfFile = PdfPages(outDir + chartTitle + '_HS_Fire_extreme10' + ownersToGraph[0] + '.pdf')
    else:
        ownersToGraph = [ownership]
        pdfFile = PdfPages(outDir + chartTitle + '_HS_Fire_extreme10' + ownersToGraph[0] + '.pdf')
...
#                            dataList.append(ownerArea[' Stand Replacing FirePMG3'].iloc[0] + ownerArea[' Stand Replacing FirePMG4'].iloc[0] + ownerArea[' Stand Replacing FirePMG5'].iloc[0])
                ...
                if scenario == runList[0]:
                    ...
                    indexTable = dataTable.head(n=10)
                ...
                plotList.append(extremeList)

        yLabelText = 'Top 10 extreme HS fire years in FF landscape (1000 ha)'
        reporterFunc.plotReporter6_bw(fig, ax, '', pdfFile, plotList, runList, yLabelText)

        pdfFile.savefig()
        pl.close()
    pdfFile.close()
    print "Done with PMG in HS fire extreme 10."
