...
def main(subArea, runName, chartTitlePre, ownership):
    ...
    varList = [' High Suitability']
    yLabelText = 'Northern Spotted Owl (VFO) High Suitability (ha)'
    ...
    if ownership == 'All':
        ...
        pdfFile = PdfPages(outDir + 'report2_NSO_VFO_All.pdf')
    else:
        ownersToGraph = [ownership]
        pdfFile = PdfPages(outDir + 'report2_NSO_VFO_' + ownersToGraph[0] + '.pdf')
    ...
    for scenario in ['CurrentPolicy','NoFedTreat','Restoration','NoFireNoMgmt']:
        ...
        if os.path.isdir(inDir):
            yearList = list(set(pd.io.parsers.read_csv(inDir + r'VFO_Spotted_Owl_pivot.csv')[' Year']))
            repList = list(set(pd.io.parsers.read_csv(inDir + r'VFO_Spotted_Owl_pivot.csv')[' Run']))
            totalArea = pd.io.parsers.read_csv(inDir + r'VFO_Spotted_Owl_pivot.csv')
