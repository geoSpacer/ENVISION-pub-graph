...
def main(outDir, subArea, chartTitlePre):
    ...
    repList = list(set(pd.io.parsers.read_csv(outDir + r'PotentialDisturbance_pivot.csv')[' Run']))
    for repNum in repList:
        pdfFile = PdfPages(outDir + 'R' + str(repNum) + '_PotentialDisturbance.pdf')
        ...
        totalArea = pd.io.parsers.read_csv(outDir + r'PotentialDisturbance_pivot.csv')
