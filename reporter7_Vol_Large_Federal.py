...
def main(outDir, runList, statsFile):
    ...
    for scenario in runList:
        ...
        if os.path.isdir(inDir):
            totalArea = pd.io.parsers.read_csv(inDir + r'LgTreeVol_TotalMerchHarv_D3_D55_by_OWNER_pivot.csv')
            ...
            for year in range(1,max(yearList) + 1):
                ...
                for rep in repList:
                    ...
                    dataList.append(ownerArea[' LgTreeVol TotalMerchHarv D55_1 (m3)'].iloc[0] + ownerArea[' LgTreeVol TotalMerchHarv D55_2 (m3)'].iloc[0] + ownerArea[' LgTreeVol TotalMerchHarv D55_3 (m3)'].iloc[0] + ownerArea[' LgTreeVol TotalMerchHarv D55_4 (m3)'].iloc[0] + ownerArea[' LgTreeVol TotalMerchHarv D55_5 (m3)'].iloc[0])
            ...
            appendFrame = pd.DataFrame(data=[{'Policy': scenario, 'VarGroup': 'EcoServices', 'Variable': 'CumulativeTotalMerchHarv_LgD55_m3', 'Year1': dataTable[dataTable['timeStep'] == 1]['cum_mean'].iloc[0], 'Year25': dataTable[dataTable['timeStep'] == 25]['cum_mean'].iloc[0], 'Year49': dataTable[dataTable['timeStep'] == 49]['cum_mean'].iloc[0]}])
            outTable = outTable.append(appendFrame, ignore_index = True)

    outTable.to_csv(outDir + statsFile, mode='a', header=False)
    print "Done with harvest vol large trees by owner."
