...
#                                for varName in varList:
    #                            totalArea.loc[list(ownerArea.index)[0],varName] += tempArea[varName].iloc[0]
            ...
            appendFrame = pd.DataFrame(data=[{'Policy': scenario, 'VarGroup': 'EcoServices', 'Variable': 'AbovegroundCarbon_Mg', 'Year1': dataTable[dataTable['timeStep'] == 1]['mean'].iloc[0], 'Year25': dataTable[dataTable['timeStep'] == 25]['mean'].iloc[0], 'Year49': dataTable[dataTable['timeStep'] == 49]['mean'].iloc[0]}])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        print "Usage: reporter_.py <outDir> <runList> <statsList>"
        sys.exit(1)
